...
* 插入点位
*/
class Insertpoint extends Backend
{
    ...
    public function index()
    {
    	if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                $issd=$params['application_type']=='sd';
                unset($params['application_type']);
                if ($issd) {
                    //实单
                    //判断会员现金币是否充足 
                    $user_finance=model("FinancePersonalaccount")
                            ->where(['user_code'=>$this->auth->getUserInfo()['user_code'],'type'=>0])
                            ->find();

                    $lv_value=model("FinanceParameter")
                          ->where(['group_name'=>'zcje','param_name'=>$params['grade_code']])
                          ->find();
                    if ($user_finance['balance']<$lv_value['param_value']) {
                        //会员现金币不足
                        $this->error(__('Insufficient account amount'));
                    }
                }
                ...
                $sunModel=$this->model
                            ...
                            ->where('user_area',$params['user_area'])
                            ->find();
                if ($sunModel) {
                    $sunModel['manage_user_code']=$params['user_code'];
                    $sunModel['user_area']='A';
                    $sunModel->save();
                }
                ...
                if ($issd) {
                    //实单
                    //操作报单
                    $params['param_business_code'] = Engine::BUSCODE_HYBD;
                    $params['param_user_code'] =$this->auth->getUserInfo()['user_code'];
                    $params['param_counterparty']=$params['user_code'];
                    $params['param_transaction_amount'] = $lv_value['param_value'];
